...
function pocaoCurar(id){

    const pokemon = pokemons[id]
    const pocao = 20

    if(pokemon.hp >= 0) pokemon.hp = pokemon.hp + pocao
    
    if(pokemon.hp >= 100) pokemon.hp = 100

    return `HP do ${pokemon.nome} após a poção: ${pokemon.hp}`
}

module.exports = {salvarPokemons, mostrarPokemon, mostrarPokemons, atualizarPokemon, deletarPokemon, batalhaPokemon, pocaoCurar}
